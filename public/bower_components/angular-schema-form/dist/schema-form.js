...
try {
  //This throws an expection if module does not exist.
    module.get('angularSpectrumColorpicker');
  deps.push('angularSpectrumColorpicker');
} catch (e) {console.log(e)}



    var schemaForm = module.get('schemaForm', deps);




//module.get('schemaForm').provider('sfPath', [function() {
module.get('schemaForm').provider('sfPath', [function() {
  ...
  // if we're on Angular 1.2.x, we need to continue using dot notation
    //Modification custom pour angualr car la version utilise est est superieur a 1.3
...
module.get('schemaForm').provider('sfBuilder', ['sfPathProvider', function(sfPathProvider) {
...
module.get('schemaForm').provider('schemaFormDecorators', ['$compileProvider', 'sfPathProvider', function($compileProvider, sfPathProvider) {
        ...
        return {
          ...
          link: function(scope, element, attrs, sfSchema) {
            ...
            scope.buttonClick = function($event, form) {
              if (_.isFunction(form.onClick)) {
                form.onClick($event, form);
              } else if (_.isString(form.onClick)) {
                if (sfSchema) {
  ...
  var createManualDirective = function(type, templateUrl, transclude) {
    transclude = _.isUndefined(transclude) ? transclude : false;
    $compileProvider.directive('sf' + _.uppercase(type[0]) + type.substr(1), function() {
      return {
        restrict: 'EAC',
        scope: true,
        replace: true,
          ...
          _.forEach(attrs, function(value, name) {
            if (name[0] !== '$' && name.indexOf('ng') !== 0 && name !== 'sfField') {
              ...
              var updateForm = function(val) {
                if (_.isUndefined(val) && val !== form[name]) {
                  form[name] = val;

                  //when we have type, and if specified key we apply it on scope.
                  if (once && form.type && (form.key || _.isUndefined(attrs.key))) {
                    scope.form = form;
                    once = false;
                  }
                }
  ...
  this.createDecorator = function(name, templates) {
    ...
    _.forEach(templates, function(url, type) {
  ...
  this.defineDecorator = function(name, fields) {
    ...
    _.forEach(fields, function(field, type) {
      field.builder = field.builder || [];
      field.replace = _.isUndefined(field.replace) ? field.replace : true;
...
module.get('schemaForm').provider('sfErrorMessage', function() {
 ...
 */
module.get('schemaForm').provider('schemaForm', ['sfPathProvider', function(sfPathProvider) {
  ...
  var canonicalTitleMap = function(titleMap, originalEnum) {
    if (!_.isArray(titleMap)) {
      var canonical = [];
      if (originalEnum) {
        _.forEach(originalEnum, function(value, index) {
          canonical.push({name: titleMap[value], value: value});
        });
      } else {
        _.forEach(titleMap, function(name, value) {
  ...
  var stdFormObj = function(name, schema, options) {
    ...
    var f = options.global && options.global.formDefaults ?
            _.clone(options.global.formDefaults) : {};
  ...
  this.$get = function() {
    ...
    service.merge = function(schema, form, ignore, options, readonly, asyncTemplates) {
      ...
      return postProcessFn(form.map(function(obj) {
        ...
        if (obj.itemForm) {
          ...
          _.forEach(stdForm.items, function(item) {
            var o = _.copy(obj.itemForm);
            o.key = item.key;
            obj.items.push(o);
          });
        }
        ...
        if (obj.key) {
          ...
          if (lookup[strid]) {
            ...
            _.forEach(schemaDefaults, function(value, attr) {
        ...
        if (obj.tabs) {
          _.forEach(obj.tabs, function(tab) {
    ...
    service.traverseSchema = function(schema, fn, path, ignoreArrays) {
      ignoreArrays = ! _.isUndefined(ignoreArrays) ? ignoreArrays : true;

      path = path || [];

      var traverse = function(schema, fn, path) {
        fn(schema, path);
        _.forEach(schema.properties, function(prop, name) {
    ...
    service.traverseForm = function(form, fn) {
      fn(form);
      _.forEach(form.items, function(f) {
        service.traverseForm(f, fn);
      });

      if (form.tabs) {
        _.forEach(form.tabs, function(tab) {
          _.forEach(tab.items, function(f) {
            service.traverseForm(f, fn);
          });
        });
      }
 ...
 *
 */
module.get('schemaForm').factory('sfSelect', ['sfPath', function(sfPath) {
  var numRe = /^\d+$/;
...
/* global tv4 */
module.get('schemaForm').factory('sfValidator', [function() {
 ...
 */
module.get('schemaForm').directive('sfArray', ['sfSelect', 'schemaForm', 'sfValidator', 'sfPath',
        ...
        var once = scope.$watch(attrs.sfArray, function(form) {
            ...
            if (form.items.length > 1) {
              subForm = {
                ...
                items: form.items.map(function(item) {
                  ...
                  if (_.isUndefined(item.readonly)) {
          ...
          scope.appendToArray = function() {
            ...
            schemaForm.traverseForm(copy, function(part) {
              ...
              if (part.key) {
                var def;
                if (!_.isUndefined(part['default'])) {
                  def = part['default'];
                }
                if (! _.isUndefined(part.schema) &&
                    ! _.isUndefined(part.schema['default'])) {
 ...
 */
module.get('schemaForm').directive('sfChanged', function() {
  return {
    require: 'ngModel',
    restrict: 'AC',
    scope: false,
...
module.get('schemaForm').directive('sfField', ['$parse', '$compile', '$http', '$templateCache', '$interpolate', '$q', 'sfErrorMessage',
     'sfPath','sfSelect',
    function($parse,  $compile,  $http,  $templateCache, $interpolate, $q, sfErrorMessage,
             sfPath, sfSelect) {
...
module.get('schemaForm').directive('sfMessage', ['$injector', 'sfErrorMessage', function($injector, sfErrorMessage) {
      ...
      var update = function(checkForErrors) {
        if (checkForErrors) {
          if (!scope.hasError()) {
            ...
            var errors = [];
            _.forEach(scope.ngModel && scope.ngModel.$error, function(status, code) {
              if (status) {
 ...
 * Directive that handles the model arrays
 */
module.get('schemaForm').directive('sfNewArray', ['sfSelect', 'sfPath', 'schemaForm', function(sel, sfPath, schemaForm) {
  return {
    scope: false,
    link: function(scope, element, attrs) {
      scope.min = 0;
      ...
      var onChangeFn =  function() {
        if (scope.form && scope.form.onChange) {
          if (_.isFunction(scope.form.onChange)) {
        ...
        if (scope.form && scope.form.schema && scope.form.schema.items) {
          ...
          if (items.type && items.type.indexOf('object') !== -1) {
            ...
            if (!scope.options || scope.options.setSchemaDefaults !== false) {
              empty = ! _.isUndefined(items['default']) ? items['default'] : empty;
...
module.get('schemaForm').directive('sfSchema', ['$compile', '$http', '$templateCache', '$q','schemaForm', 'schemaFormDecorators', 'sfSelect', 'sfPath', 'sfBuilder',
    ...
    return {
      ...
      link: function(scope, element, attrs, formCtrl, transclude) {
        ...
        scope.$on('schemaFormRedraw', function() {
          var schema = scope.schema;
          var form   = scope.initialForm ? _.clone(scope.initialForm) : ['*'];
          if (schema) {
            render(schema, form);
          }
        });
...
module.get('schemaForm').directive('schemaValidate', ['sfValidator', '$parse', 'sfSelect',
    ...
    return {
      ...
      link: function(scope, element, attrs, ngModel) {
        ...
        ['$validators', '$asyncValidators'].forEach(function(attr) {
          ...
          if (form[attr] && ngModel[attr]) {
            _.forEach(form[attr], function(fn, name) {
